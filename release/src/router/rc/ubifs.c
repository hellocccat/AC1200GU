...
void start_ubifs(void)
{
...
#ifdef RTCONFIG_MTK_NAND
	if (!mtd_getinfo(UBIFS_VOL_NAME, &mtd_part, &mtd_size)) return;

	_dprintf("*** ubifs: %s(%d, %d)\n", UBIFS_VOL_NAME, mtd_part, mtd_size);

	/* try to attach ubi */
	_dprintf("*** ubifs: attach\n");
	...
	if (ubi_getinfo(UBIFS_VOL_NAME, &dev, &part, &size) == 1) {	//ubi volume not found, format it and create volume
		...
		if (vol_size > 0) {
			...
			if (!mtd_unlock(UBIFS_VOL_NAME)) {
				error("unlocking");
				return;
			}

			/*if (!check_if_file_exist(UBI_DEV_PATH)) {
				_dprintf("*** ubifs: format\n");
				eval("ubiformat", MTD_DEV_PATH, "-y");	
			}*/
