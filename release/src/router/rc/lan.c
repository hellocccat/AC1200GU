...
gen_qca_wifi_cfgs(void)
{
	...
	foreach (wif, nvram_safe_get("lan_ifnames"), next) {
		...
		else if (!strcmp(wif, STA_2G) || !strcmp(wif, STA_5G))
		{
			if(nvram_get_int("sw_mode") == SW_MODE_REPEATER){
#if defined(RTCONFIG_CONCURRENTREPEATER)
				if ((repeater_mode() && nvram_get_int("wlc_express") == 0) || mediabridge_mode())	/* don't bine linux bridge */
					sprintf(!strcmp(wif,"sta0") ? cmd : cmd1,"wpa_supplicant -B -P /var/run/wifi-%s.pid -D athr -i %s -c /etc/Wireless/conf/wpa_supplicant-%s.conf",wif,wif,wif);
				else	/* bind linux bridge (-b br0) */
					sprintf(!strcmp(wif,"sta0") ? cmd : cmd1,"wpa_supplicant -B -P /var/run/wifi-%s.pid -D athr -i %s -b br0 -c /etc/Wireless/conf/wpa_supplicant-%s.conf",wif,wif,wif);
#else	/* RTCONFIG_CONCURRENTREPEATER */
	...
	if(nvram_get_int("sw_mode") == SW_MODE_REPEATER){
		...
		if (strlen(cmd)) {
			doSystem(cmd);
			doSystem("iwpriv sta0 extap 1");
			doSystem("iwpriv sta0 shortgi 1");
			doSystem("ifconfig sta0 up");
			if (nvram_match("wlc0_ssid", ""))
				eval("wpa_cli", "-p", "/var/run/wpa_supplicant-sta0", "-i", "sta0", "disconnect");
		}

		if (strlen(cmd1)) {
			doSystem(cmd1);
			doSystem("iwpriv sta1 extap 1");
			doSystem("iwpriv sta1 shortgi 1");
			doSystem("ifconfig sta1 up");
			if (nvram_match("wlc1_ssid", ""))
				eval("wpa_cli", "-p", "/var/run/wpa_supplicant-sta1", "-i", "sta1", "disconnect");
		}
#else	  /* RTCONFIG_CONCURRENTREPEATER */
		doSystem(cmd);
#if 0 // ARPNAT is obsolete
...
#if defined(RTCONFIG_CONCURRENTREPEATER)
                if (nvram_get_int("sw_mode") == SW_MODE_REPEATER) {
                        if (((nvram_get_int("wlc_express") - 1) == unit && sunit == 0) || sunit > 0)
                                continue;
                }
