...
define platformKernelConfig
	...
	if [ "$(RALINK)" = "y" ]; then \
		...
		if [ "$(MT7621)" = "y" ]; then \
			if [ "$(RPAC56)" = "y" ] ; then \
				...
				sed -i "/CONFIG_PCIE_PORT2/d" $(1); \
				echo "CONFIG_PCIE_PORT2=y" >>$(1); \
				sed -i "/CONFIG_RALINK_HSDMA/d" $(1); \
				echo "CONFIG_RALINK_HSDMA=m" >>$(1); \
		...
		if [ "$(RTN54U)" = "y" ] && [ $(BOOT_FLASH_TYPE) = "SPI" ] ; then \
			sed -i "//d" $(1); \
			echo "CONFIG_MTD_SPI_FAST_CLOCK=y" >>$(1); \
		fi; \
	...
	if [ "$(RTAC1200GA1)" = "y" ] || [ "$(RTAC1200GU)" = "y" ]; then \
		...
		echo "CONFIG_MTD_ANY_RALINK=y" >>$(1); \
	else \
		sed -i "/CONFIG_MTD_ANY_RALINK/d" $(1); \
	...
	if [ "$(FIRST_IF)" = "MT7603E" ] && [ "$(RPAC56)" = "" ] ; then \
	...
	if [ "$(SECOND_IF)" = "MT7612E" ]  && [ "$(RPAC56)" = "" ] ; then \
		...
		echo "# CONFIG_QOS_DLS_SUPPORT is not set" >>$(1); \
		...
		echo "# CONFIG_BLOCK_NET_IF is not set" >>$(1); \
        ...
        if [ "$(RALINK_DFS)" = "y" ] ; then \
                ...
                if [ "$(SECOND_IF)" = "MT7610E" ] ; then \
                        sed -i "/CONFIG_MT7610_AP_DFS/d" $(1); \
                        echo "CONFIG_MT7610_AP_DFS=y" >>$(1); \
                fi; \
        fi; \
	if [ "$(REPEATER)" = "y" ] ; then \
			sed -i "/CONFIG_MT_APCLI_SUPPORT/d" $(1); \
			echo "CONFIG_MT_APCLI_SUPPORT=y" >>$(1); \
			sed -i "/CONFIG_MT_MAC_REPEATER_SUPPORT/d" $(1); \
			echo "CONFIG_MT_MAC_REPEATER_SUPPORT=y" >>$(1); \
		if [ "$(RTAC53)" = "y" ] ; then \
